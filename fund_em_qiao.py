...
def fund_market_price_2() -> pd.DataFrame:#-> pd.DataFrame 这里意思就是这个表达式返还值是指向PD数据类型
    """
    新浪财经-行情中心-基金-ETF基金行情&LOF基金行情
    http://vip.stock.finance.sina.com.cn/mkt/#etf_hq_fund
    :return: 当前时刻的所有ETF基金&LOF基金市价情况
    ...
    url = f"http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeDataSimple?page=1&num=10000&sort=symbol&asc=1&node=etf_hq_fund&_s_r_a=page"
    res = requests.get(url, headers=headers)
    text_data = eval(res.text)#执行一个字符串表达式，并返回表达式的值。
    temp_df_1 = pd.DataFrame(text_data)#创建数据单元
    temp_df_1.columns = [
        "_",
        "基金名称",
        "最新价",
    ...
    text_data = eval(res.text)
    '''
    eval功能可谓非常强大，即可以做string与list,tuple,dict之间的类型转换，还可以做计算器使用！更有甚者，可以对她能解析的字符串都做处理，而不顾忌可能带来的后果！
...
def fund_value_market_estimationprice(fund: str = "165508") -> pd.DataFrame:
    ...
    b = ast.literal_eval(a.group(1))#默认还是json，
    temp_df = pd.DataFrame([b])#b转成列表类型，然后再创建数据单元
    '''
    如果不转换的话会触发异常 ValueError: If using all scalar values, you must pass an index，
    因为pandas 的dataframe需要一个可迭代的对象换成列表 或者加个index就可以解决这个问题
    '''
    temp_df.columns = [
        "基金代码",
        "基金名称",
    ...
    b = ast.literal_eval(a.group(1))#会判断需要计算的内容计算后是不是合法的python类型，如果是则进行运算，否则就不进行运算。
...
def fund_CLOSEEND_market_price() -> pd.DataFrame:
    ...
    res = requests.get(url, params=params, headers=headers)
    text_data = res.text
    a = text_data.strip("jQuery1124008820987276391623_1592103802228(")##去头去尾巴，strip() 方法用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列
    a = a.strip(");")
    data_json = demjson.decode(a)#转成JSON格式
    b = data_json["data"]
...
def fund_em_financial_fund_daily() -> pd.DataFrame:
    ...
    try:
        ...
        text_data = r.text
        '''print(text_data)
        print(text_data[text_data.find("{"): -1])#找到第一个{的位置，然后截取到-1的位置所有内容
        jQuery18306581512555641693_1588248310204(
        {"Data":{"chars":["g","h","j","n","x","y","z"],"count":[0,0,0,0],
        "record":0,"pages":"0","pagecount":0,"curpage":"1","indexsy":["-0.20","-0.10","0.05",""],
        "showday":["2020-09-30","2020-09-29"],"List":[]},"ErrCode":0,"ErrMsg":null,"TotalCount":0,"Expansion":null,"PageSize":0,"PageIndex":0})
        得出结果：
        {"Data":{"chars":["g","h","j","n","x","y","z"],"count":[0,0,0,0],
        "record":0,"pages":"0","pagecount":0,"curpage":"1","indexsy":["-0.20","-0.10","0.05",""],
        "showday":["2020-09-30","2020-09-29"],"List":[]},"ErrCode":0,"ErrMsg":null,"TotalCount":0,"Expansion":null,"PageSize":0,"PageIndex":0}
